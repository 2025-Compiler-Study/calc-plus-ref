...
이제 언어 스펙에서 사용자 지정 함수를 추가한다. 개발자가 직접 함수를 선언해서 사용할 수 있다.
...
전체적인 문법 흐름은 기존과 비슷해보이지만 바뀐부분이 꽤 있다.  
기존 구현 코드가 잘 호환되지 않을 수도 있다는 점을 확인해야한다.

* `VAR` 대신 `IDENT`를 사용한다. 기존 `VAR`는 글자만 가능했지만 이제 숫자도 넣을 수 있다.
* `read()`, `write()`를 더 이상 문법의 요소로 취급하지 않는다.
* 이제 함수 호출이 `expr`의 일부로 취급된다. 인자가 없는 경우부터, 여러개 추가도 가능하다.
* 함수 정의가 가능하다. `func` 키워드로 시작하는 것을 제외하면 C언어 문법처럼 함수를 정의한다.
* 기존 `stmt` 안에 `return`문이 추가된다.
* 코드는 모든 함수 정의들로만 구성되며, 전역변수 개념은 없다.

### 구현 과제 #1

각 함수 정의를 해석해서, 함수 별 AST를 만들어야 한다.  
기존 구현을 바탕으로 수정/추가 예상되는 AST 노드는 아래와 같다.

1. return 문
2. 함수 호출 일반화 (built-in read/write 변경 혹은 wrap 필요)
3. 함수 정의 자체를 묶어주는 노드
4. 함수 매개변수 선언 및 값 전달 (함수 호출자의 책임일수도, 함수 정의의 책임일수도 있음)

일단은 해당 AST 노드를 추가하고, AST builder가 변환하는 기능만 작성한다.

### 구현 과제 #2

`func main() {}` 함수가 인터프리터 실행의 시작 지점이 되게 한다.  
아직 다른 함수를 정의하거나, `read()`, `write()`는 연결하지 않아도 된다.

1. cli의 실행 인자 전달 기능은 구현하지 않아도 된다. (C의 `int argc, char* argv[]`)
2. `main`의 매개변수는 없는 것으로 가정해도 된다.
3. main의 return type은 int여도 되고, void여도 된다. 제한해도 되고, 둘 다 구현해도 된다.
4. AST builder가 만든 함수 정의 AST 중 이름이 `main`인 것은 단 하나여야 한다.  
   불만족시 오류를 내고 종료해야 한다.

고려사항:
`main`이 매개변수를 허용한다면, 첫 실행 지점에서는 각 인자로 0들이 전달되었다고 가정한다.

### 구현과제 #3

이제 `read()`, `write()` 함수를 지원해야 한다.

1. 함수의 시그니쳐 및 동작 방식은 기존 calc3에서 추가된 함수들과 동일하다.
2. `write`의 반환값 처리 방식을 결정한다.
3. `write`의 함수 반환값을 읽는 경우를 확인한다.
4. AST builder가 만든 함수 정의 AST 중 이름이 `read`, `write`인 것은 존재해선 안된다.  
   오류를 내고 종료해야 한다.
5. `main` 함수에서 각 내장 함수들을 호출해서 결과가 달라지는 코드를 작성하고, 실행해서 검증한다.

고려사항:
`write`는 원래 값을 반환하는지 궁금하지 않게 문법에서 호출 가능한 방법을 제한해왔다.  
이제는 문법 제한이 풀려버렸으므로, 반환값 처리가 필요하다.
`void` 반환으로 강제해도 되고, `int` 반환이지만 0을 반환하게 해도 된다.  
이 부분의 핵심은 함수 호출 및 구현에 집중하라는 것이다.

### 구현과제 #4

이제 사용자 정의 함수를 지원해야 한다.

1. 위의 `main`, `read`, `write` 이름이 아닌 함수를 만들어야 한다.
2. 함수 정의에서 선언한 매개변수 개수와 함수 호출의 매개변수가 다르다면 오류를 내고 종료해야 한다.
3. 함수가 다른 함수를 호출하는 코드를 실행해보자.
4. 여러 함수 호출 depth에서 built-in 함수 처리도 확인해보자.
5. 직접 재귀, 간접 재귀를 구현해보자.
6. return 문에서 함수를 호출하거나 재귀호출하는 코드도 확인해보자. (ex. factorial)

힌트: 
각 함수가 호출되어서 실행할 때, 변수를 보관하던 심볼 테이블이  
각 실행중인 함수의 lifecycle마다 개별 관리되어야 한다.  
return 문을 만나면 현재 함수 실행을 끝내고 호출한 쪽으로 값을 돌려줘야 한다.

### 보조 과제 

구현 과제처럼 프로그래밍을 할 필요는 없지만, 잘 생각해보고 답변해 볼 것

1. 왜 `ReadAssign`, `Write` 문법 부분을 제거해야 했을까?
2. AST를 구현하지 않고도 사용자 함수 정의 및 호출을 구현할 수 있었을까?
